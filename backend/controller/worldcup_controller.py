...
# /select/random/cars GET Response Body
worldcup_random_model = worldcup.model(
    "worldcup_random_result",
    WorldCup.response_model_rand,
)

# /worldcup/result GET Response Body
worldcup_model = worldcup.model(
    "worldcup_result",
...
class Select_Random_Cars(Resource):
    @worldcup.response(200, "Success", worldcup_random_model)
    def get(self):
...
class Worldcup_Result(Resource):
    ...
    @worldcup.response(200, "Success", worldcup_model)
    @worldcup.response(404, "후보에 해당하는 차량이 없습니다.")
    def get(self):
